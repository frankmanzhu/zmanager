...
impl CanonicalTree {
    ...
    pub fn create_instance_allow_empty(&self) -> Result<Option<Tree>, DecodeFailed> {
        ...
        let largest_length = match NonZeroU8::new(*self.path_lengths.iter().max().expect("empty path lengths")) {
            Some(x) => x,
        ...
        for bit in 1..=largest_length.get() {
            ...
            for code in 0..self.path_lengths.len() {
                ...
                if self.path_lengths[code] == bit {
                    huffman_tree.get_mut(pos..pos + amount).ok_or(DecodeFailed::InvalidPathLengths)?.iter_mut().for_each(|x| *x = code as u16);
        ...
        Ok(Some(Tree { path_lengths: self.path_lengths.clone(), largest_length, huffman_tree }))
    }
    ...
    pub fn create_instance(&self) -> Result<Tree, DecodeFailed> {
        self.create_instance_allow_empty()?.ok_or(DecodeFailed::EmptyTree)
    }

    // Note: the tree already exists and is used to apply the deltas.
    pub fn update_range_with_pretree(&mut self, bitstream: &mut Bitstream, range: Range<usize>) -> Result<(), DecodeFailed> {
        ...
        while i < range.end {
            ...
            match code {
                ...
                17 => {
                    ...
                    self.path_lengths.get_mut(i..i + zeros as usize + 4).ok_or(DecodeFailed::InvalidPretreeRle)?.iter_mut().for_each(|x| *x = 0);
                    i += zeros as usize + 4;
                }
                18 => {
                    let zeros = bitstream.read_bits(5)?;
                    self.path_lengths.get_mut(i..i + zeros as usize + 20).ok_or(DecodeFailed::InvalidPretreeRle)?.iter_mut().for_each(|x| *x = 0);
                    i += zeros as usize + 20;
                }
                19 => {
                    ...
                    self.path_lengths.get_mut(i..i + same as usize + 4).ok_or(DecodeFailed::InvalidPretreeRle)?.iter_mut().for_each(|x| *x = value);
                    i += same as usize + 4;
                }
...
impl fmt::Debug for Tree {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Tree").field("path_lengths", &self.path_lengths.len()).field("largest_length", &self.largest_length).finish()
    }
}
...
mod tests {
    ...
    fn decode_complex_table() {
        ...
        let tree = Tree::from_path_lengths(vec![1, 0, 0, 0, 0, 7, 3, 3, 4, 4, 5, 5, 5, 7, 8, 8, 0, 7, 0, 0]).unwrap();
        let value_count = vec![(0, 128), (6, 32), (7, 32), (8, 16), (9, 16), (10, 8), (11, 8), (12, 8), (5, 2), (13, 2), (17, 2), (14, 1), (15, 1)];
