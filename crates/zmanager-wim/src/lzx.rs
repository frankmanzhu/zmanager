...
//! Private WIM LZX decoder derived from the `lzxd` crate.
//!
//! The implementation retains the original MIT/Apache-2.0 licensing and is
//! intentionally not part of zmanager-wim's public API.
...
/// WIM's default LZX block size.
pub(crate) const WIM_DEFAULT_BLOCK_SIZE: u32 = 32 * 1024;

/// Maximum chunk supported by this decoder until dynamic chunk storage is added.
pub(crate) const MAX_WIM_CHUNK_SIZE: usize = MAX_CHUNK_SIZE;
...
pub(crate) struct DecoderState {
    ...
    /// Decode WIM-dialect streams rather than CAB/`[MS-PATCH]` streams.
    wim_mode: bool,

    /// Block size implied by WIM's "default size" flag: the chunk size the
    /// containing WIM declares.
    wim_default_block_size: u32,
}
...
impl fmt::Display for DecodeFailed {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            OverreadBlock => write!(f, "read more items than available in the block in a single step"),
            ...
            ChunkTooLong => write!(f, "tried reading a chunk longer than {} bytes", MAX_CHUNK_SIZE),
        }
    }
}
...
/// WIM applies the E8 filter with this fixed translation size rather than
/// reading one from a stream header (wimlib calls it `LZX_WIM_MAGIC_FILESIZE`).
const WIM_E8_TRANSLATION_SIZE: i32 = 12_000_000;
...
impl Lzxd {
    ...
    pub fn new(window_size: WindowSize) -> Self {
        ...
        Self {
            ...
            state: DecoderState { window_size, main_tree, length_tree, wim_mode: false, wim_default_block_size: 0 },
            ...
            current_block: Block { remaining: 0, size: 0, kind: BlockKind::Uncompressed { r: [1, 1, 1] } },
        }
    }

    /// Creates a decoder for the LZX dialect used by Windows Imaging (WIM)
    /// resources.
    ///
    /// WIM streams differ from the CAB/`[MS-PATCH]` streams [`Lzxd::new`]
    /// decodes in three ways, all handled here:
    ///
    /// 1. There is no leading "E8 translation" flag bit, so decoding starts at
    ///    the first block header rather than one bit into the stream.
    /// 2. Block sizes use a 1-bit "default size" flag with a 16-bit fallback,
    ///    not an unconditional 24-bit field.
    /// 3. The E8 call-translation filter is applied unconditionally with a
    ///    fixed translation size of 12,000,000, rather than being gated on the
    ///    (absent) header flag.
    ///
    /// Each WIM chunk is compressed independently, so use one decoder per
    /// chunk rather than carrying state across them.
    #[must_use]
    pub fn new_wim(window_size: WindowSize) -> Self {
        let mut this = Self::new(window_size);
        this.state.wim_mode = true;
        this.state.wim_default_block_size = WIM_DEFAULT_BLOCK_SIZE;
        // There is no header to read; suppress the CAB-only leading flag bit.
        this.first_chunk_read = true;
        this.postprocess = Some(PostProcessState { data_chunk: vec![0; MAX_CHUNK_SIZE].into_boxed_slice(), e8_translation_size: WIM_E8_TRANSLATION_SIZE });
        this
    }
    ...
    fn try_read_first_chunk(&mut self, bitstream: &mut Bitstream) -> Result<(), DecodeFailed> {
        ...
        if !self.first_chunk_read {
            ...
            self.postprocess = if e8_translation {
                Some(PostProcessState { data_chunk: vec![0; MAX_CHUNK_SIZE].into_boxed_slice(), e8_translation_size: bitstream.read_bits(32)? as i32 })
            } else {
                None
            };
        }
    ...
    fn postprocess(translation_size: i32, chunk_offset: usize, idata: &mut [u8]) -> Result<&[u8], DecodeFailed> {
        let mut processed = 0usize;

        // Find the next E8 match, or finish once there are no more E8 matches.
        while let Some(pos) = idata[processed..].iter().position(|&e| e == 0xE8).map(|pos| processed + pos) {
            ...
            let abs_val = i32::from_le_bytes([idata[pos + 1], idata[pos + 2], idata[pos + 3], idata[pos + 4]]);
            if (abs_val >= -(current_pointer as i32)) && abs_val < translation_size {
                let rel_val = if abs_val.is_positive() { abs_val.wrapping_sub(current_pointer as i32) } else { abs_val.wrapping_add(translation_size) };
    ...
    pub fn decompress_next(&mut self, chunk: &[u8], output_len: usize) -> Result<&[u8], DecompressError> {
        ...
        while decoded_len != output_len {
            if self.current_block.remaining == 0 {
                ...
                if matches!(self.current_block.kind, BlockKind::Uncompressed { .. }) && self.current_block.size % 2 != 0 {
                    bitstream.read_byte();
                }
            ...
            let decoded = self.current_block.decode_element(&mut bitstream, &mut self.r)?;
        ...
        if let Some(postprocess) = self.postprocess.as_mut() {
            ...
            if chunk_offset >= 0x4000_0000 || decoded_len <= 10 {
                ...
                let view = Self::postprocess(postprocess.e8_translation_size, chunk_offset, postprocess_buf)?;
                Ok(view)
            }
        } else {
            Ok(view)
    ...
    pub fn reset(&mut self) {
        let mut this = Self::new(self.state.window_size);
        if self.state.wim_mode {
            this.state.wim_mode = true;
            this.state.wim_default_block_size = self.state.wim_default_block_size;
            this.first_chunk_read = true;
            this.postprocess = Some(PostProcessState { data_chunk: vec![0; MAX_CHUNK_SIZE].into_boxed_slice(), e8_translation_size: WIM_E8_TRANSLATION_SIZE });
        }
...
mod tests {
    ...
    fn check_uncompressed() {
        let data = [0x00, 0x30, 0x30, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, b'a', b'b', b'c', 0x00];
    ...
    fn check_e8() {
        let data = [
            ,
        ];
    ...
    fn uncompressed_advance_zero() {
        ...
        let data = vec![0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32, 0x32];
    ...
    fn peek_bits_oneword_bounds_check() {
        ...
        // Data from fuzzer crash
        let data = vec![
            0x12, 0x12, 0x12, 0x12, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x12, 0x12, 0x12, 0x12, 0x12, 0x12, 0x12, 0x12, 0x12, 0x00, 0x00,
            0x00, 0x12, 0x8a,
        ];
