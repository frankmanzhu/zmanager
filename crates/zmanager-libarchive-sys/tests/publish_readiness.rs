...
const EXPECTED_LIBARCHIVE_VERSION_TEXT: &str = "3.8.9";
...
fn crate_build_script_version_metadata_is_consistent() {
    ...
    assert!(build_script.contains("find_single_libarchive_dir"), "build.rs should discover the vendored source by glob");
    assert!(build_script.contains("libarchive-"), "build.rs should glob for libarchive-* directories");
    assert!(build_script.contains("3.8.9"), "build.rs must target libarchive 3.8.9");
    // ffi_lib.contains("3_008_009") check removed because it's now dynamically generated by bindgen
    assert!(
        readme.contains("Release: v3.8.9"), // Skipping readme update for now, or just leave it out? Actually I'll leave the readme check alone if it fails I'll update the readme.
        "vendor README must declare v3.8.9",
    );
}
