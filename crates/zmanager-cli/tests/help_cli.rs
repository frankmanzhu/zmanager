...
const TEST_HELP_NEEDLES: &[&str] =
    &["Verify archive readability", "zm test <archive>", "--include <glob>", "--public-no-key", "--json", FILTER_GLOB_NOTE, "printf '%s\\n'"];

const PLAN_HELP_NEEDLES: &[&str] =
    &["Show what create would archive", "zm plan <paths...>", "--files-from <file|->", "--exclude-from <file>", FILTER_GLOB_NOTE, "zm plan project/"];
...
fn top_level_help_is_user_facing_and_hides_legacy_commands() {
    ...
    assert!(
        !stdout.contains("zip-create") && !stdout.contains("Legacy development commands") && !stdout.contains("help legacy"),
...
fn package_preview_uploads_artifacts_without_publishing_release() {
    for required in [
        ...
        "path: dist/zm-full-${{ matrix.target }}.*",
    ...
    for required in ["--features", "New-FullReleasePackage", "zm-full-$TargetTriple.zip"] {
        assert_contains(CI_WINDOWS_PS1, required);
    }
}
...
fn linux_release_artifacts_are_static_tarballs() {
    ...
    for required in
        ["target: x86_64-unknown-linux-musl", "target: aarch64-unknown-linux-musl", "musl-tools", "sha256sum *.tar.gz *.zip *.deps.txt > SHA256SUMS"]
    {
        assert_contains(RELEASE_WORKFLOW, required);
    }
